...
function useAddTokenToWallet(): [(tokenAddress: string, symbol: string, image?: string) => Promise<void>, boolean] {
  const [loading, setLoading] = useState(false);
  const provider = useProvider() as ethers.providers.JsonRpcProvider;

  const fn = useCallback(async (tokenAddress: string, symbol: string, _image?: string) => {
    if(loading) return;
              ...
              <tbody>
                <tr>
                  <td className="flex flex-col items-enter">
                    ...
                    <Button
                      ...
                      onClick={() => addTokenToWallet(addresses.JewelToken, "JEWEL", DFKJewel)}
                ...
                <tr>
                  <td className="flex flex-col items-enter">
                    ...
                    <Button
                      className="text-xs p-1 mr-auto"
                      onClick={() => addTokenToWallet(addresses.gmJEWEL, "gmJEWEL", DFKLockedJewel)}
                      disabled={addingTokenToWallet}
                    >
                      Add to Wallet
                    </Button>
                ...
                <tr>
                  <td className="flex flex-col items-enter">
                    ...
                    <Button
                      className="text-xs p-1 mr-auto"
                      onClick={() => addTokenToWallet(addresses.GMGToken, "GMG", SmallGMG)}
                      disabled={addingTokenToWallet}
                    >
                      Add to Wallet
                    </Button>
