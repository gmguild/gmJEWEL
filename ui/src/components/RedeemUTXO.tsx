...
export function RedeemUTXO() {
  ...
  const formattedSelectedUTXOValue = useFormattedBigNumber(selectedUTXO?.newVal);
  ...
  const formattedCostToBuy = useMemo(() => {
    ...
    return (
      <span className={classNames(colour)}>{BigNumberToFloat(costToBuy).toFixed(3)} gmJEWEL</span>
    );
  }, [costToBuy, gmBalance]);
  ...
  const redeemStash = () => redeemUTXO().then(() => { forceRefreshAllUTXOs(); setSelectedUTXO(null); });
  const redeemDisabled = doing || (!!gmBalance && !!costToBuy && BigNumber.from(gmBalance).lt(costToBuy))
    ...
    <div className="max-w-5xl mx-auto font-lora p-4">
      <article className={classNames("py-6 prose")}>
        <p className="flex flex-row text-center mx-auto">
          Your Balance (gmJEWEL) is <span className="ml-2 text-gray-900 flex flex-row items-center"><div className="jewel-icon h-4 w-4 mr-1" /> {!loadingInformation ? formattedGmBalance : '...loading...'}</span>
        </p>
      </article>
      ...
      <div className="w-full grid grid-cols-12 my-4">
          ...
          <div className={classNames(
            ...
            {!selectedUTXO && (
              <p className="italic">No Stash selected</p>
            )}
            {selectedUTXO && (
              <>
                ...
                <p className="flex flex-row mb-1">
                  <span className="text-gray-500">Value</span> <span className="ml-2 text-gray-900 flex flex-row items-center"><div className="jewel-icon h-4 w-4 mr-1" /> {!loadingInformation ? formattedSelectedUTXOValue : '...loading...'}</span>
                </p>

                <p className="flex flex-row mb-2">
                <span className="text-gray-500">Cost</span> <span className="ml-2 text-gray-900 flex flex-row items-center"><div className="jewel-icon h-4 w-4 mr-1" /> {!loadingInformation ? formattedCostToBuy : '...loading...'}</span>
                </p>

                <Button className="mx-auto" disabled={redeemDisabled} onClick={redeemStash}>Redeem Stash</Button>
              </>
            )}
          </div>
        ...
        <div className={classNames(
          ...
          {loadingAllUTXOs && <p className="text-center">...loading stashes...</p>}
          {allUTXOs.map((utxo, i) => {
            return (
              <JewelStash key={i} selected={!!selectedUTXO && (selectedUTXO.utxoAddress === utxo.utxoAddress)} onClick={() => setSelectedUTXO(utxo)} utxo={utxo} />
            );
          })}
        </div>
