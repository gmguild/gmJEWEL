...
const SwapAssetPanel = ({
  ...
  return (
    <div className="rounded-[14px] border border-taupe-200 hover:border-taupe-100 bg-taupe-400 p-3 flex flex-col gap-4">
      {header({
        disabled,
        onChange,
        value,
  ...
  const content = (
    ...
    >
      ...
      <Typography
        ...
        weight={700}
        component="span"
        className="flex items-center gap-1 px-2 py-1 rounded-full cursor-pointer text-high-emphesis hover:text-white hover:shadow bg-taupe-300 hover:bg-taupe-200"
      >
        Wallet
      </Typography>
    </Typography>
...
> = ({ walletToggle, currency, onSelect, spendFromWallet, id, currencies }) => {
  const trigger = currency ? (
    <div
      id={id}
      className="flex items-center gap-2 px-2 py-1 rounded-full shadow-md cursor-pointer text-high-emphesis bg-taupe-300 hover:bg-taupe-200"
    >
      <CurrencyLogo
        currency={currency}
