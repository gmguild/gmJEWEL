...
const _selectTxStatus = createSelector(
  ...
  (
    ...
    web3Context: Web3ReactContext,
    txHash
  ): TransactionStatus => {
    const tx = web3Context.chainId
      ? transactions[web3Context.chainId]?.[txHash]
      : undefined;
    return !tx?.receipt
      ? "PENDING"
      : tx &&
...
export const selectTxStatus = (txHash: string) => (state: AppState) =>
  _selectTxStatus(state, txHash);
